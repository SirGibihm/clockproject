...
void generateDisplayDefault() {
  ...
  manipulateDisplay(defaultDisplay, 9);
...
int generateDisplayHour(int hour) {
  ...
  if(hour%12 == 0){
    ...
    int defaultDisplay[] = {52, 53, 62, 61, 60};
    manipulateDisplay(defaultDisplay, 4);
  }
}
...
void generateDisplayMinute(int minute) {
  ...
  if(minute/10 == 1){
    ...
    int defaultDisplay[] = {84, 83, 82, 81, 80, 79, 78,};
...
void loop() {
  ...
  // set the time as a demo application
  for (int hour = 0; hour < 24; hour++) {
    for (int minute = 0; minute < 60; minute++)  {
      generateDisplay(hour, minute);
      FastLED.show();
